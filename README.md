...
Drag or use arrow keys to rotate, pinch with two fingers, scroll, or use +/− to zoom, and press 0 to reset. Mobile graph labels use a smaller font; one finger rotates and two fingers zoom without rotating. Top-down view hides height; counts remain in the people list. Every top-20 poster is plotted. One post can support a directional estimate. Weak or ambiguous evidence is placed near the center independently on each axis; missing evidence or legacy null scores use 50 (the graph origin), with uncertain fallbacks labeled in the details. A centered position does not necessarily mean an explicitly neutral view. Starting analysis clears the previous graph and people. The graph panel shows discovered posters with animated, deduplicated retrieved-post counts (up to 20 cards), then a model-analysis stage with red cards for active requests, green cards for completed assessments, a completion counter, and elapsed time. Each request assesses one poster, with at most two concurrent requests and a five-minute timeout per request (retrieval and model listing retain two-minute timeouts). This sends up to 20 model requests and repeats the prompt for each, which can increase token costs; there are no automatic retries. The active color indicates an outstanding request, not visibility into the model’s internal reasoning. The finished graph appears only after validated results arrive. Failed or cancelled analysis leaves a stopped progress view; retry or load a new demo crowd. Reduced-motion preferences disable the animations.
...
- Model responses require bounded scores or null, confidence, rationale, and evidence. Quotes must match text actually supplied from that author. Missing verifiable evidence produces low-confidence center placements on both axes; a null score centers only that axis when evidence exists. These are subjective estimates of views expressed in this thread, not facts about a person's overall beliefs.
...
## Session axis presets

The default remains AI potential versus outlook, with retrieved post count as height. Under **Create or edit axes**, describe two opinion dimensions and generate a definition with the selected provider/model. Model loading and axis generation do not require a thread URL. Review and edit the preset name, endpoint labels, scoring criteria, and center meanings, then choose **Save & use preset**. You can also edit a selected definition into a new preset without an LLM call.

Presets live only in page memory and disappear on refresh. Changing presets clears previous scores; analyze the thread again with the new criteria. Each analysis returns its own validated definition snapshot. Graph labels, score details, and accessibility descriptions use that definition. Short endpoint labels remain visible while zooming. Axis generation is one extra billable request; thread analysis still uses up to 20 poster requests. No credentials or presets are written to browser storage. Views are inferred only from the supplied thread, not the poster’s overall identity.

## Session usage

The small Session indicator below the status expands to show input/output token counts and costs by selected provider/model. It updates on every inference response (including invalid model output) and covers axis generation and thread analysis. Retrieval and model-list requests are excluded. All records are in memory and reset on refresh; they contain no keys or post text.

OpenAI-compatible token usage and Anthropic input/output/cache token usage are supported. OpenRouter's documented `usage.cost` is displayed as reported USD cost. Other providers currently show cost unavailable; no hard-coded price table or guessed conversion is used. Pending requests and incomplete/unreported usage are flagged, including cancelled or failed requests whose charges may not be returned. Any available cost subtotal is labeled partial when some costs are missing. This is not a complete billing statement and excludes credit-purchase fees and taxes.

Provider format references: [OpenRouter usage accounting](https://openrouter.ai/docs/cookbook/administration/usage-accounting), [OpenRouter USD credits](https://openrouter.ai/support/), [Anthropic cache usage](https://platform.claude.com/docs/en/build-with-claude/prompt-caching).
