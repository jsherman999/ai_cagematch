...
export function providerSettings(input) {
  return { base: providerBase(input.endpoint), key: credential(input.apiKey, 'LLM API key') };
}
export async function listModels(input, { request = jsonRequest, signal } = {}) {
  const { base, key } = providerSettings(input);
  parseThreadURL(input.url);
  const data = await request(`${base}/models`, { headers: { Authorization: `Bearer ${key}` }, signal, service: 'LLM model list' });
  if (!Array.isArray(data.data)) throw new AppError('The provider must expose an OpenAI-compatible GET /models endpoint.', 502);
  const models = [...new Set(data.data.map(m=>m.id).filter(id=>typeof id==='string' && id.length<=200))].sort();
...
export async function analyze(input, { request=jsonRequest, signal, onProgress=()=>{} }={}) {
  ...
  const result=await request(`${base}/chat/completions`,{
    method:'POST',headers:{Authorization:`Bearer ${key}`},signal,service:'LLM analysis',
    body:{model:input.model,stream:false,messages:[{role:'system',content:instructions},{role:'user',content:JSON.stringify({authors:supplied})}]}
  });
  const choice=result.choices?.[0];
  if (choice?.finish_reason==='length') throw new AppError('The model response was cut off. Choose a model with a larger output limit.',502);
  if (choice?.message?.refusal) throw new AppError('The model declined to analyze this thread.',502);
  let raw;
  try { raw=JSON.parse(choice.message.content.replace(/^\s*```(?:json)?\s*/,'').replace(/\s*```\s*$/,'')); }
