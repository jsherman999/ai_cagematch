import { trackInference } from './usage.js';
...
export async function generateAxes(input,{request=jsonRequest,signal,onUsage}={}){
 const {base,headers,kind}=providerSettings(input);
 ...
 request=trackInference(request,input,onUsage);
...
export async function analyze(input, { request=jsonRequest, signal, onUsage, onPost=()=>{}, onAssessment=()=>{}, onProgress=()=>{} }={}) {
  const { base,headers,kind }=providerSettings(input);
  if (typeof input.model!=='string' || !input.model.trim() || input.model.length>200) throw new AppError('Choose a model.');
  request=trackInference(request,input,onUsage);
